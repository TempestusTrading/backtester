...
impl Strategy for SMACrossoverStrategy {
    fn on_ticker(&mut self, ticker: &Ticker, broker: &mut Broker) -> Result<(), StrategyError> {
        ...
        if let Ok(sma) = self.sma_indicator.get_value() {
            if sma > ticker.close && self.previous_sma < self.previous_ticker.as_ref().unwrap().close {
                broker.submit_order(
                    self.order_id,
                    Order {
                ...
                self.order_id += 1;
            } else if sma < ticker.close && self.previous_sma > self.previous_ticker.as_ref().unwrap().close {
                broker.submit_order(
                    self.order_id,
                    Order {
                        symbol: "AAPL".to_string(),
